        ...
        <>
          <TableContainer component={Paper}>
            <Table>
              ...
              <TableBody>
                {paginatedAppointments.map((appt) => {
                  // 🕒 Combine session date & time into one Date object
                  const appointmentDateTime = new Date(
                    `${appt.session_date} ${appt.session_time}`
                  );

                  const now = new Date();
                  const timeDiff = appointmentDateTime - now;
                  const lessThanFiveHours = timeDiff < 5 * 60 * 60 * 1000;
                    ...
                    <TableRow key={appt.id}>
                      ...
                      <TableCell>
                        <Tooltip
                          title={
                            lessThanFiveHours
                              ? "Cannot cancel within 5 hours of appointment time"
                              : ""
                          }
                        >
                          <span>
                            <Button
                              variant="outlined"
                              color="error"
                              disabled={isDisabled || lessThanFiveHours}
                              onClick={async () => {
                                const confirmed = await Swal.fire({
                                  title: "Cancel Appointment?",
                                  text: "Are you sure you want to cancel this appointment?",
